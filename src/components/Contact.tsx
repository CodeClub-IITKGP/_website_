...
import { 
  User, Mail, Phone, MapPin, Globe, Facebook, 
  Instagram, MessageSquare, Clock, Send, GraduationCap, 
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 relative">
        ...
        >
          ...
          <p className="text-xl md:text-2xl text-gray-400 max-w-4xl mx-auto leading-relaxed">
            This is the team of CodeClub IIT Kharagpur. Meet the dedicated faculty and students who drive our mission forward.
          </p>
        </motion.header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div className="space-y-6">
                ...
                <p className="text-lg text-gray-300 leading-relaxed">
                  I am working as an Associate Professor in the Department of Computer Science and Engineering at the Indian Institute of Technology (IIT) Kharagpur.
                </p>
                <div className="space-y-4">
                  <p className="text-gray-400 leading-relaxed">
                    I obtained my Bachelor of Engineering (BE) degree from Jadavpur University, Kolkata in 2009 and Master of Technology (M Tech) and Doctor of Philosophy (Ph.D.), both from IIT Guwahati, in 2011 and 2014, respectively.
                  </p>
                  <p className="text-gray-400 leading-relaxed">
                    As a system researcher, my primary research interests is in the intersection of computer systems, human computer interaction and machine learning.
                  </p>
                </div>
              ...
              <div className="relative group aspect-square lg:aspect-auto h-full min-h-[400px]">
                <div className="absolute inset-0 bg-gray-900 border border-white/10 rounded-3xl flex flex-col items-center justify-center text-gray-600 transition-all duration-500 group-hover:border-blue-500/30 overflow-hidden">
                  <div className="text-8xl mb-4 group-hover:scale-110 transition-transform duration-700 opacity-40">👨‍🏫</div>
        ...
        {/* Contact Information */}
        <section className="mb-40">
          <SectionHeader title="Get In Touch" color="text-blue-400" desc="Have questions or want to join our community? Reach out to us through any of these channels." />
          ...
          <div className="grid lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
            ...
            >
              ...
              <form className="space-y-6">
                <InputField label="Name" placeholder="Your full name" type="text" />
                <InputField label="Email" placeholder="your.email@example.com" type="email" />
                <div>
                  <label className="block text-gray-400 mb-3 text-sm font-medium">Message</label>
                  <textarea
                    rows={5}
            ...
            <div className="space-y-8">
              ...
              >
                <h3 className="text-2xl font-bold text-purple-300 mb-8">Contact Details</h3>
                <div className="space-y-6">
                  <ContactItem icon={<Mail className="text-blue-400" />} label="Email" value="To be filled" />
                  <ContactItem icon={<Phone className="text-green-400" />} label="Phone" value="To be filled" />
                  <ContactItem icon={<MapPin className="text-purple-400" />} label="Location" value="Department of CSE, IIT Kharagpur" />
                </div>
              </motion.div>
        ...
        <section className="mb-20">
          <SectionHeader title="🌐 Connect With Us" color="text-yellow-400" desc="Follow us on social media and stay updated with our latest activities, events, and achievements." />
          ...
          <div className="max-w-5xl mx-auto">
            <div className="bg-gradient-to-br from-blue-900/20 via-purple-900/10 to-transparent rounded-[2.5rem] p-10 md:p-16 border border-white/10">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <SocialCard icon={<Instagram />} label="Instagram" color="purple" />
                <SocialCard icon={<MessageSquare />} label="WhatsApp" color="green" />
              </div>
              <div className="mt-16 text-center max-w-3xl mx-auto">
                <p className="text-gray-400 text-lg md:text-xl leading-relaxed italic">
                  Join our community and be part of the exciting journey in competitive programming, workshops, and cultural events at IIT Kharagpur!
                </p>
              </div>
            </div>
          </div>
...
const SectionHeader = ({ title, color, desc }: any) => (
  <div className="text-center mb-20">
    ...
    {desc && <p className="text-xl text-gray-400 max-w-4xl mx-auto leading-relaxed">{desc}</p>}
  </div>
);

const Tag = ({ text, color }: any) => (
  <span className={`${color} text-white px-4 py-1.5 rounded-full text-sm font-medium shadow-lg shadow-black/20`}>
    {text}
  </span>
);
...
const MemberCard = ({ color, role, year, details, compact }: any) => {
  const colorMap: any = {
    purple: "hover:border-purple-500/40 hover:shadow-[0_0_30px_-10px_rgba(168,85,247,0.3)] bg-purple-600",
    green: "hover:border-green-500/40 hover:shadow-[0_0_30px_-10px_rgba(34,197,94,0.3)] bg-green-600"
  };

  return (
    <motion.div 
      whileHover={{ y: -8 }}
      className={`bg-white/[0.02] rounded-[2rem] ${compact ? 'p-6' : 'p-8'} border border-white/5 transition-all duration-300 ${colorMap[color].split(' bg')[0]}`}
    >
      <div className="text-center mb-6">
        <div className={`w-20 h-20 ${colorMap[color].split('hover:')[1].split(' ')[1]} rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner`}>
      ...
      <div className="space-y-4 pt-4 border-t border-white/5 text-center">
        ...
        <div className="bg-gray-900/50 rounded-2xl p-4 border border-white/5">
          <p className="text-gray-500 text-xs tracking-wide uppercase">{details}</p>
        </div>
      </div>
    </motion.div>
  );
...
const InputField = ({ label, placeholder, type }: any) => (
  <div>
    <label className="block text-gray-400 mb-3 text-sm font-medium">{label}</label>
    <input
      type={type}
...
const ContactItem = ({ icon, label, value }: any) => (
  <div className="flex items-center gap-5 group">
    ...
    <div>
      <p className="text-sm font-mono text-gray-500 uppercase tracking-wider">{label}</p>
...
const HourItem = ({ day, time, isClosed }: any) => (
  <div className="flex justify-between items-center py-2 border-b border-white/5 last:border-0">
    ...
    <span className={isClosed ? "text-red-400 text-sm font-bold uppercase" : "text-gray-500 font-mono text-sm"}>
      {time}
    </span>
  </div>
);
...
const SocialCard = ({ icon, label, color }: any) => {
  const glow = color === "blue" ? "hover:border-blue-500/40 hover:shadow-blue-500/20" : 
               color === "purple" ? "hover:border-purple-500/40 hover:shadow-purple-500/20" : 
               "hover:border-green-500/40 hover:shadow-green-500/20";
  const bg = color === "blue" ? "bg-blue-600" : color === "purple" ? "bg-purple-600" : "bg-green-600";
  ...
  return (
    ...
    >
      <div className={`w-14 h-14 ${bg} rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg`}>
        {icon}
      </div>
      <h3 className="font-bold text-gray-200 text-sm">{label}</h3>
      <p className="text-[10px] text-gray-600 mt-2 font-mono uppercase">To be filled</p>
    </motion.div>
  );
};
